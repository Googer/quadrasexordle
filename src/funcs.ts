...
// Given a quadrasexordle id, return the corresponding target wordles
...
export function loadSettingsFromLocalStorage(dispatch: Dispatch) {
  const text = localStorage.getItem("quadrasexordle-settings");
...
export function saveSettingsToLocalStorage(state: SettingsState) {
  localStorage.setItem("quadrasexordle-settings", JSON.stringify(state));
}
